...
class MainClass {
//Methods
	public static string StringToBinary(string data) //Convert a string to binary
	{
	...
	public static string BinaryToString(string data) //Convert binary to a string
	{
  ...
  public static void Main (string[] args) {
    ...
    List<string> inven = new List<string>();
		...
		string[] combosAndRecipesArray = new [] {
			"fire+fire", "energy", "land+land", "continent", "solarsystem+solarsystem", "galaxy",
			"earth+earth", "dirt",	"continent+continent", "globe", "galaxy+galaxy", "*universe*",
			"earth+energy", "earthquake", "globe+atmosphere", "planet", "sun+pressure", "*blackhole*",
			"water+earth", "mud", "water+water", "ocean", "wind+wind", "*tornado*",
			"water+dirt", "mud",	"dust+dust", "minerals", "clay+fire", "brick",
			"mud+fire", "clay",	"fire+planet", "sun", "brick+brick", "wall",
			"energy+air", "wind", "sun+cloud", "sky", "wall+wall", "home",
			"wind+earth", "dust", "ocean+minerals", "amino acids", "home+home", "village",
			"fire+water", "steam", "aminoacids+sun", "life", "village+village" + "city",
			"water+air", "mist", "water+land", "river", "city+city", "metropolis",
			"steam+air", "cloud", "water+continent", "ocean", "metropolis+metropolis", "population",
			"fire+air", "pressure", "energy+cloud", "storm",
			"earth+air", "dust", "storm+energy", "lightning",
			"dirt+dirt", "land", "cloud+water", "rain",
			"air+air", "atmosphere", "lava+water", "rock",
			"earthquake+earth", "mountain", "wind+rock", "dust",
			"earth+fire", "lava", "water+dust", "cloud",
			"lava+mountain", "volcano", "planet+sun", "solar system",
			"volcano+pressure", "eruption", "planet+planet", "solar system"
    ...
    bool running = true;
		...
		//Variables regarding login and saving data
		string username = "";
		string password = "";
		bool loggedIn = false;
    ...
    string[] possCraftCodes = new string[3] {"c", "crafting", "craft"};
		...
		string[] possLoginCodes = new [] {"login", "l"};
			//Possible input codes for accessing the login menu
      ...
      if (possExitCodes.Contains(input)) {
        ...
        break;
      	...
      	input = Console.ReadLine().ToLower().Replace(">> ", "").Replace(" ", "");
				...
				Console.Write("\nPress ENTER to continue");
				Console.ReadLine();
			} else if (possLoginCodes.Contains(input)) {
				Console.WriteLine("Login to save and load your progress:");
				Console.Write("USERNAME: ");
				username = Console.ReadLine().ToLower().Replace("USERNAME: ", "").Replace(" ", "");
				Console.Write("PASSWORD: ");
				password = Console.ReadLine().ToLower().Replace("PASSWORD: ", "").Replace(" ", "");
